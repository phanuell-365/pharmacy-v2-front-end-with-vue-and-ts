...
const outOfStock: Ref<MedicineOutOfStockDto[]> = ref([]);
const expiredStock: Ref<ExpiredMedicineDto[]> = ref([]);
...
try {
  ...
  outOfStock.value = await medicinesStore.fetchMedicinesOutOfStock();
  expiredStock.value = await medicinesStore.fetchExpiredMedicines();
