    ...
    <form autocomplete="off" novalidate @submit.prevent>
        ...
        <div class="row g-3">
          <div class="col-lg-9 col-md-9">
            <div class="col-lg-4 col-md-4">
              <SearchPanel :filter-array="customerSearchPairs" class="mx-2" label="customer"
                           name="customer" @on-found-item="onFoundCustomerItem" />
            </div>
          </div>
          <div class="col-lg-3 col-md-3">
            <div class="row my-3 me-3">
              <div class="col-lg-12 col-md-12 mb-3 text-end">
                ...
                <h4 class="fs-4 fw-bold text-muted">Kshs. {{ amountReceived }}</h4>
              </div>
              <div class="col-lg-12 col-md-12">
                <label class="lead fs-6">Amount: </label>
                <input id="amount" v-model="amountReceived" class="form-control" name="amount" type="text">
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-lg-12 col-md-12">
            <div class="row my-1 align-baseline">
              ...
              <!--              <template v-else>-->
              <div class="col-lg-4 col-md-4">
                <SearchPanel ref="medicineSearchPanelRef" :filter-array="medicineSearchPairs" label="medicine"
                             name="medicine"
              ...
              <InputContainer :invalid-feedback="issueUnitQuantityErrorMessage" input-id="issueUnitQuantity"
                ...
                <input id="validationIssueUnitQuantity" ref="issueUnitQuantityRef" v-model="issueUnitQuantity"
                       ...
                       :disabled="!addMode" class="form-control"
              ...
              <div class="col-lg-4 col-md-4 m-auto mt-4 pt-3">
                <FormButton
                  :disabled="!issueUnitQuantityMeta.validated && !issueUnitQuantityMeta.valid"
                  skin="primary" text="Add Item"
                  @click="addMedicineToSelectedMedicine" />
                <FormButton skin="info" text="add" @click="onShowAddItemModalClick" />
              </div>
              <!--              </template>-->
            </div>
          </div>
          <div class="col-lg-12 col-md-12">
            <TableContainer skin="light">
              <TableHead>
                <PlainTableHeader :field-names="customersOrdersAttributes" scope="col" />
                <th>Actions</th>
              </TableHead>
              <TableBody>
                <TableRow :col-count="customersOrdersAttributes.length + 1" :records="customersOrders"
                          null-comment="No medicine selected">
                  <template #default="{record}">
                    <PlainTableData :field-names="customersOrdersAttributes" :record="record" />
                    <td>
                      <div class="text-center">
                        ...
                        <!--                        </template>-->
                        <!--                        <template v-else>-->
                        <ButtonIcon :icon-name="UPDATE_ICON" class="mx-1"
                                    skin="secondary" @click="onSetUpdateModeClick(record.id)" />
                        <ButtonIcon :icon-name="DELETE_ICON" class="mx-1" skin="danger"
      ...
      <Teleport to="body">
        ...
        <ActionModal ref="updateCustomerOrderModal">
          <template #modal-body>
            <div class="row">
              <p class="fs-5 fw-semibold">Please enter the new issue unit quantity</p>
              <div class="row mb-3">
                <label class="col-sm-2 col-form-label fs-6 fw-semibold" for="colFormLabel">Quantity</label>
                <div class="col-sm-10 m-auto">
                  <input id="validationUpdateIssueUnitQuantity" ref="newIssueUnitQuantityRef"
                         v-model="newIssueUnitQuantity"
                         :autofocus="updateMode"
                         :class="{'is-invalid': !newIssueUnitQuantityMeta.valid && newIssueUnitQuantityMeta.validated}"
                         autocomplete="off"
                         ...
                         required
                         type="text">
                  <div class="invalid-feedback">{{ newIssueUnitQuantityErrorMessage }}</div>
...
customerSearchPairs.value = customers.value.map(value => ({ name: value.name, id: value.id }));
medicineSearchPairs.value = medicines.value.map(value => ({ name: value.name, id: value.id }));
...
const onFoundCustomerItem = (item: SearchPairs) => {
  ...
  foundCustomer.value = customers.value.find(value => value.id === foundCustomerId.value);
};
...
const onFoundMedicineItem = (item: SearchPairs) => {
  ...
  foundMedicine.value = medicines.value.find(value => value.id === foundMedicineId.value);
};
...
const addMedicineToSelectedMedicine = () => {
  ...
  if (customersOrdersStore.getItemNames.includes(foundMedicine.value?.name as string) || !foundMedicine.value) toastError.value?.show();
  else
...
const customerOrderItemToUpdate: Ref<CustomersOrders | undefined | null> = ref(null);
...
const onSetUpdateModeClick = (medicineOrderId: string) => {
  ...
  customerOrderItemToUpdate.value = customersOrdersStore.getItems.find(value => value.id === medicineOrderId);

  newIssueUnitQuantity.value = customerOrderItemToUpdate.value?.quantity.toString() as string;
...
const onUpdateCustomerOrder = () => {
  ...
  if (newIssueUnitQuantityMeta.valid && newIssueUnitQuantityMeta.validated) {
    ...
    newIssueUnitQuantityErrorMessage.value = "Please enter the issue unit quantity!";
  }
  // issueUnitQuantity.value = "";
...
const onShowAddItemModalClick = () => {
  addItemModal.value?.setFocus();
};
</script>
<style scoped>
#validationUpdateIssueUnitQuantity, #validationIssueUnitQuantity {
  width: 250px;
}
</style>
